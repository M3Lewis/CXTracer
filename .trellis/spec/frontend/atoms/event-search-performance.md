...
- Raise a custom event from the ViewModel when asynchronous/debounced filtering completes, and handle it in the View to scroll the selection back into view:
  ...
  ```
- Bind original source text to a custom attached property, keeping the built-in `Text` property unbound in XAML to avoid binding nullification loops during keyword highlighting:
  ```xml
