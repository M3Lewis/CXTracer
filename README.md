...
- **SukiUI 7.0.1** (基于橙色主题 `Orange` 的精美扁平化设计，支持中英文动态多语言切换)
...
* **长日志快速阅读**：为了解决长日志（如 Assistant 完整回复、命令行长输出或复杂 Tool 荷载）在窄列分栏下阅读吃力的问题，CXTracer 支持**双击任意消息卡片**打开详情浮层。而**单击消息卡片**用于选中该卡片，高亮显示以准备键盘导航或同步滚动定位。
* **暗色遮罩与 Raw JSON**：浮层采用半透明暗色遮罩背景，卡片主体采用对应角色的主题背景。底部配有可折叠的 **Raw JSON** 展开器，方便对原始 JSON 报文进行精确排查。按 `Esc` 键、点击空白区域或点击右上角关闭按钮即可退出浮层。

### 6. 全局多语言动态切换 (i18n Support)
* **无感动态切换**：支持英文 (English) 与简体中文双语切换。用户可在 Settings 窗口即时更改，所有静态文本、计算文本（会话数、事件数）、复制 Toast 及下拉过滤器名称等将即时通过 Avalonia 的 `MergedDictionaries` 重载更新，无需重启应用。
* **智能区域检测**：首次启动（无本地配置文件）时，应用会智能检测当前操作系统的 UI 区域语言（如检测到 `zh` 区域则默认进入中文界面）。
* **完全 AOT 兼容**：多语言设置作为持久化属性保存在 `%LOCALAPPDATA%\CXTracer\settings.json` 中，读写与动态切换逻辑均不依赖任何反射，完美契合 Native AOT 裁剪构建。
...
项目配置了完整的 Native AOT 和 Trim 裁剪支持（配置于 `CXTracer.csproj`）。Native AOT 编译需要目标系统的编译器支持，因此各平台通常需在对应的系统环境中执行发布。

#### 1. 前置依赖

* **Windows**: 安装 Visual Studio 并勾选 "使用 C++ 的桌面开发" 工作负载（包含 MSVC 编译器和 Windows SDK）。
* **macOS**: 安装 Xcode 命令行工具：
  ```bash
  xcode-select --install
  ```
* **Linux (以 Ubuntu 为例)**: 安装 Clang 编译器及开发库：
  ```bash
  sudo apt-get update
...
* **双重独立检索**：
  ...
  * **日志检索（右侧）**：右侧顶栏的 ComboBox 左侧新增了内容过滤框，专职针对当前会话中的日志事件内容进行关键字查找过滤。整个过滤采用多线程异步分批过滤与防抖，极大降低大日志检索的性能消耗。
* **分类视图切换**：提供 `All` / `Conversation` / `Commands` / `Errors` / `Diffs` / `Final` / `Tools` / `Raw` 快速过滤。
* **单击与双击交互改变**：
  * **单击消息卡片**：选中当前卡片，高亮展示，并将键盘焦点移回主窗口（即使键盘焦点原本在日志过滤框中，也会释放）。此时，用户可立即使用 `Up/Down` 方向键在当前过滤列表内导航。
  * **双击消息卡片**：打开该消息的详情弹窗浮层。
* **检索过滤与全局导航的无缝切换**（检索 -> 选中 -> 清空 -> 全局导航）：
  1. 在日志检索框中输入关键字，列表将自动显示匹配的过滤项。
  2. 单击（或通过 `Up/Down` 键）选中目标卡片。即使此时正在检索框内输入，方向键也会直接转移控制至列表，无需手动切换焦点。
  3. 点击检索框右侧的清除按钮 `(x)` 清空检索词。
  4. 此时，**当前卡片的选中高亮状态会自动保留**，且列表将平滑滚动，使该选中卡片**自动对齐至视口最上方**。
  5. 此时直接按下 `Up/Down` 键，即可立即以该卡片为起点，顺畅地向后或向前进行全局上下文事件的键盘导航。
* **卡片绝对序号指示器**：
  * 每张 Conversation 和 Execution 卡片右上角显示两个精致的圆形胶囊标识。
  * **左边胶囊**：显示该卡片在其所在单栏内的绝对时间顺序序号。
  * **右边胶囊**：显示该卡片在双栏时间线合并后的绝对全局序号。
  * 胶囊序号为日志文件的绝对坐标，不受当前关键字检索或分类筛选的影响，始终保持稳定不漂移，极大方便调试时的信息定位。
...
本项目基于 **GNU Affero General Public License v3.0 (AGPL-3.0)** 协议开源。详细协议内容请参阅 [LICENSE](LICENSE) 文件。
